// consniffer is a connection sniffing utility
package main
	...
	for {
		...
		if ok {
			...
			case ethernetdecode.Ipv4Header:
				...
				packetsize = iphdr.(ethernetdecode.Ipv4Header).Len
			...
			case ethernetdecode.Ipv6Header:
				...
				packetsize = iphdr.(ethernetdecode.Ipv6Header).PayloadLength
			}
			...
			if tcphdr.DestinationPort == uint16(*tcpPort) || tcphdr.SourcePort == uint16(*tcpPort) {
				...
				log.Printf("Seq: %d, Ack: %d", tcphdr.SeqNum, tcphdr.AckNum)

				log.Printf("Ethernet payload size = %d, TCP packet size = %d", n, packetsize)
